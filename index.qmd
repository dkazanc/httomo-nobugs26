...
<object data="images/mpi_chunks_reslice_oriented.svg"
        type="image/svg+xml"
...
<object data="images/httomo_reslicing_dark.svg"
        type="image/svg+xml"
        style="display:block; width:100%; aspect-ratio:16/9; margin:auto;">
</object>
...
<object data="images/httomo_gpu_section.svg"
        type="image/svg+xml"
        style="display:block; width:100%; aspect-ratio:16/9; margin:auto;">
</object>
...
<object data="images/gpu_memory_estimation_vector.svg"
        type="image/svg+xml"
        style="display:block; width:100%; aspect-ratio:16/9; margin:auto;">
</object>
...
<object data="images/httomo_features.svg"
        type="image/svg+xml"
        style="display:block; width:100%; aspect-ratio:16/9; margin:auto;">
</object>
...
## Building HTTomo pipeline with YAML templates {.small}
...
## Exercise 1: pre-processing (d/f correction) {.small}
...
3. Add [saving](https://diamondlightsource.github.io/httomo/howto/process_lists/save_results/save_results.html#example-1-save-output-of-a-specific-method) into the HDF5 file the result of **minus_log**.
...
## Exercise 2: pre-processing (dezingering) {.small}
...
3. Add [saving](https://diamondlightsource.github.io/httomo/howto/process_lists/save_results/save_results.html#example-1-save-output-of-a-specific-method) into the HDF5 file the result of **remove_outlier3d**.
4. Validate the YAML file with [YAML checker](https:/K/diamondlightsource.github.io/httomo/utilities/yaml_checker.html):
```bash
python -m httomo check pipeline_synth.yaml tomodata_synth.nxs
```
...
## Centre of Rotation (CoR) estimation {#cor_estimation .smaller}
...
## Why use Open MPI in HTTomo? {#openmpi-benefits .small}
...
Re-run the pipeline from the [Excercise 5](#excercise5) but in parallel. 
...
You can monitor your CPU usage panel when executing the job. The time of the parallel execution should be smaller compared to the `-n 1` baseline for 1 process.
...
[Section](#sections) **progress counts data blocks, rather than methods or the whole pipeline.**
...
1. **`Pipeline has been separated into N sections`**   HTTomo groups methods into sections that process data blocks.
...
## HTTomo parameter sweeping {#sweep_syntax}
...
Perform a **sweep** run for the [center of rotation](#cor_estimation) using the YAML syntax provided in the [previous slide](#sweep_syntax). 
...
- Memory **peaks** at [reslice](#reslice) operation
...
::: {.fragment}
### Example: one process, 24 GB budget
```bash
httomo run \
  --max-memory 24G \
  data.nxs pipeline.yaml output/
```
**Illustrative 32 GB workstation:** allocate 24 GB to HTTomo and leave 8 GB for the operating system and other applications. Adjust to actual available RAM.
...
## GPU processing - faster libraries

HTTomo is designed to optimise GPU-accelerated processing, making it the preferred approach for achieving high throughput with supported methods.
...
![Source: Kazantsev et al., manuscript in preparation *"High-throughput GPU tomographic software HTTomo for high-performance computing at Diamond Light Source"*.](images/timing_analysis_dark.png){
  width=100%
  fig-alt="CPU vs GPU methods with transfers"
  fig-align="center"
}

## GPU processing: data management

HTTomo aims to process **as much data as safely fits in GPU memory** to reduce transfer overhead and improve GPU utilisation. The framework therefore follows two main principles:
...
- HTTomo divides data chunks into **blocks** for GPU processing.
...
::: {.fragment}
### Method chains reduce transfers

HTTomo [chains](#sections) compatible GPU methods within a **section**, keeping each block on the GPU between methods.

**For each block:**

1. Transfer the block to the GPU.
2. Run the GPU method chain.
3. Return the result when CPU access is needed.
...
## Adding a processing method to HTTomo {#own_methods}
...
Package and test `my_filters` so it is importable in the HTTomo environment.
...
### Using 4 NVIDIA RTX PRO 6000 Blackwel GPUs
